...
describe 'openssh_test::service on ubuntu-10_04' do
  ...
  context 'when using default parameters' do
    ...
    it 'steps into openssh_service and installs package[openssh-server]' do
      expect(ubuntu_10_04_default_stepinto_run).to install_package('openssh-server')
    end

    it 'steps into openssh_service and create template[/etc/ssh/sshd_config]' do
      expect(ubuntu_10_04_default_stepinto_run).to create_template('/etc/ssh/sshd_config').with(
        :owner => 'root',
        :mode => '0644'
        )
    end
    ...
    it 'steps into openssh_service and manages service[ssh]' do
      expect(ubuntu_10_04_default_stepinto_run).to start_service('ssh')
      expect(ubuntu_10_04_default_stepinto_run).to enable_service('ssh')
    end
  end
end
